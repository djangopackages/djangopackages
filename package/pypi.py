...
def update_package_from_pypi(
    ...
    *,
    ...
    save_package: bool = False,
) -> bool:
    """
    Fetch PyPI JSON metadata for a Package and update Package + Version.

    Returns True on a successful fetch + update, False when no fetch occurred or
    when the project does not exist on PyPI (404).

    Notes:
    - This function may update and save the related Version row.
    - Package is modified in-memory but NOT saved by default.
    - Set save_package=True to persist package changes.

    Args:
        package: Package model instance to update
        clear_pypi_url_on_404: If True, clear pypi_url when package not found
        save_package: If True, save the package model after updating

    Returns:
        True if package was successfully updated from PyPI, False otherwise
    """
