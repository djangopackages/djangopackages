...
def _build_commit_rollups_batch(package_ids: list[int]) -> CommitRollups:
    ...
    commits_over_52: dict[int, str] = {
        package_id: ",".join(map(str, reversed(weeks)))
        for package_id, weeks in buckets_by_package.items()
    }

    return CommitRollups(
        last_commit_date=last_commit_date, commits_over_52=commits_over_52
    )
...
def _update_commit_rollups_batched(
    ...
    total_updated = 0
    fields = ["commits_over_52", "last_commit_date"]
    ...
    with Progress(
        ...
        for package_ids_batch in _chunked_iterator(
            ...
            for package in Package.objects.filter(pk__in=package_ids_batch).only(
                ...
                package.commits_over_52 = rollups.commits_over_52.get(package.pk, "")
...
@click.option(
    "--commits/--no-commits",
    default=True,
    help="Update commits_over_52 and last_commit_date",
)
