...
class Package(BaseModel):
    ...
    @property
    def latest_pypi_version_number(self):
    ...
    @cached_property
    def commits_over_52_list(self):
        return [
            int(commit)
            for c in self.commits_over_52.split(",")
            if (commit := c.strip())
        ]
    ...
    def calculate_score(self):
        ...
        if self.last_updated:
            ...
            last_updated_penalty = (
                math.modf(delta_months / 3)[1] * self.repo_watchers / 10
            )
        else:
            last_updated_penalty = self.repo_watchers / 10
    ...
    @property
    def development_status_pretty(self):
        return self.latest_version.pretty_status if self.latest_version_id else None
    ...
    def pypi_ancient(self):
        return (
            self.latest_version.upload_time < now() - timedelta(365)
            if self.latest_version_id
            else None
        )
    ...
    def no_development(self):
        ...
        return commit_date < now() - timedelta(365) if commit_date else None
