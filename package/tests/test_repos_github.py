...
def test_fetch_commits(github_handler, package):
    assert Commit.objects.count() == 0
    github_handler.fetch_commits(package)
    assert Commit.objects.count() > 0


def test_fetch_metadata(github_handler, package):
...
def test_fetch_metadata_archived_repo(github_handler, package_archived):
...
def test_fetch_metadata_unsupported_repo(github_handler, package_invalid):
    # test what happens when setting up an unsupported repository
    assert package_invalid.repo_url == "https://example.com"
