from dataclasses import dataclass
import logging
from typing import Optional

from django.conf import settings
import httpx

from .base_handler import BaseHandler


logger = logging.getLogger(__name__)


REPOSITORY_URL = "https://codeberg.org/api/v1/repos/{repository}"
COMMITS_URL = "https://codeberg.org/api/v1/repos/{repository}/commits"
COLLABORATORS_URL = "https://codeberg.org/api/v1/repos/{repository}/collaborators"


@dataclass
class ForgejoMetadata:
    archived: bool
    archived_at: str
    description: str
    forks_count: int
    watchers_count: int


@dataclass
class ForgejoCommit:
    sha: str
    created: str


class CodebergHandler(BaseHandler):
    title = "Codeberg"
    url_regex = "(https|git)://codeberg.org/"
    url = "https://codeberg.org/"
    repo_regex = r"(?:https|git)://codeberg.org/[^/]*/([^/]*)"
    slug_regex = repo_regex

    def __init__(self):
        self.api_token = settings.CODEBERG_TOKEN

    def fetch_commits(self, package: "package.models.Package"):
        from package.models import Commit  # Added here to avoid circular imports

        for commit in _fetch_commits(package.repo_name()):
            try:
                _, created = Commit.objects.get_or_create(
                    package=package,
                    commit_hash=commit.sha,
                    commit_date=commit.created,
                )

                # If the commit record already exists, it means we are at the end of the
                # list we want to import
                # TODO: same exit condition as github, fails for allauth
                if not created:
                    break
            except Commit.MultipleObjectsReturned:
                continue

        package.save()

        return package

    def fetch_metadata(self, package: "package.models.Package"):
        repo = _fetch_repository(package.repo_name())
        collaborators = []

        for user in _fetch_collaborators(package.repo_name(), self.api_token):
            collaborators.append(user)

        if repo.archived:
            package.data_repo_archived = repo.archived_at

        package.repo_description = repo.description
        package.repo_forks = repo.forks_count
        package.repo_watchers = repo.watchers_count
        package.participants = ",".join(collaborators)
        package.save()

        return package


def _fetch_repository(repository: str) -> Optional[ForgejoMetadata]:
    try:
        url = REPOSITORY_URL.format(repository=repository)
        response = httpx.get(url)
    except Exception as exp:
        logger.error(exp)
        return None

    if response.status_code != httpx.codes.OK:
        logger.error(f"Response code: {response.status_code} for URL {url}")
        return None

    data = response.json()

    try:
        meta = ForgejoMetadata(
            archived=data["archived"],
            archived_at=data["archived_at"],
            description=data["description"],
            forks_count=data["forks_count"],
            watchers_count=data["watchers_count"],  # TODO: watchers or starts?
        )
    except KeyError:
        logger.error("Key error for URL {url}")
        return None

    return meta


def _fetch_commits(repository: str, page_size=50):
    current_page = 1

    while True:
        params = {"page": current_page, "limit": page_size}

        url = COMMITS_URL.format(repository=repository)

        try:
            response = httpx.get(url, params=params)
        except Exception as exp:
            logger.error(exp)
            break

        if response.status_code != httpx.codes.OK:
            logger.error(f"Response code: {response.status_code} for URL {url} with params {params}")
            break

        for commit in response.json():
            try:
                yield ForgejoCommit(
                    sha=commit["sha"],
                    created=commit["created"],
                )
            except KeyError:
                logger.error(f"no created timestamp for {url} with params {params}")
                break

        if "x-hasmore" in response.headers and response.headers["x-hasmore"] == "true":
            current_page = current_page + 1
        else:
            break


def _fetch_collaborators(repository: str, page_size=50, token=""):
    """TODO: right now this endpoint does not return any data."""
    if not token:
        return

    current_page = 1

    while True:
        params = {"page": current_page, "limit": page_size}

        url = COLLABORATORS_URL.format(repository=repository)

        try:
            response = httpx.get(url, params=params, headers={"Authorization": f"token {token}"})
        except Exception as exp:
            logger.error(exp)
            break

        if response.status_code != httpx.codes.OK:
            logger.error(f"Response code: {response.status_code} for URL {url} with params {params}")
            break

        for user in response.json():
            try:
                yield user["login"]
            except KeyError:
                logger.error(f"no login name for {url} with params {params}")
                break

        if "x-hasmore" in response.headers and response.headers["x-hasmore"] == "true":
            current_page = current_page + 1
        else:
            break


repo_handler = CodebergHandler()
